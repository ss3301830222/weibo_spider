...
class WbListSpider(scrapy.Spider):
    name = 'wb_list'
    id = 1380274560
    start_url = 'https://m.weibo.cn/api/container/getIndex?type=uid&value={}'.format(id)
    cookie_url = 'http://127.0.0.1:5000/weibo/random'
    weibo_url = 'https://m.weibo.cn/api/container/getIndex?uid={}&t=0&type=uid&value={}&containerid={}&page={}'
    weibo_page = 1
    weibo_page_max = 3
    repost_url = 'https://m.weibo.cn/api/statuses/repostTimeline?id={}&page={}'
    repost_page = 1
    repost_max = 10000#最多获取一百页转发
    comment_url = 'https://m.weibo.cn/comments/hotflow?id={}&mid={}&max_id_type={}'
    comment_page = 1
    comment_max = 10000#最多获取一百页评论
    attitude_url = 'https://m.weibo.cn/api/attitudes/show?id={}&page={}'
    attitude_page = 1
    attitude_max = 5000
    user_agent = [
    ...
    cookie = json.loads(requests.get(cookie_url).text)
    ...
    def parse_starinfo(self, response):
        ...
        self.weibo_url = self.weibo_url.format(id,id,item_starinfo['weibo_containerid'],'{}')
            ...
            if (len(cards) > 0):
                ...
                for j in range(len(cards)):
                    ...
                    if (card_type == 9):
                        ...
                        self.repost_page = 1
                        yield scrapy.Request(url=self.repost_url.format(item['weibo_id'], 1),cookies=self.cookie,
                        ...
                        self.attitude_page = 1
                        logging.warning("％％第{}页第{}条点赞数据开始获取％％".format(self.weibo_page, i))
                        yield scrapy.Request(url=self.attitude_url.format(item['weibo_id'],1),
                                             cookies=self.cookie,headers=self.headers,callback=self.parse_attitude,
                                             meta={'weibo_id': item['weibo_id'],'weibo_page':self.weibo_page,'weibo_i':i})
    ...
    def parse_repost(self, response):
        ...
        logging.warning("**第{}页第{}条转发数据开始获取**".format(weibo_page, weibo_i))
        logging.warning('转发页数为{}'.format(repost_max))
        ...
        try:
            for k in range(1,repost_max+1):
                ...
                yield scrapy.Request(url=next_url, callback=self.parse_repost_detail, headers=self.headers,
                                     meta={'weibo_id': weibo_id},cookies=self.cookie)
                if k % 40 == 20:
    ...
    def parse_comment(self, response):
        ...
        try:
            ...
            for i in range(len(data)):
                ...
                yield item
            self.comment_page += 1
            if self.comment_page % 20 == 10:
                self.cookie = json.loads(requests.get(self.cookie_url).text)
                logging.warning("评论更换cookie")
            if self.comment_page < self.comment_max:
                ...
                yield scrapy.Request(url=next_url, callback=self.parse_comment,headers=self.headers,
                                             meta={'weibo_id': weibo_id,'weibo_i':weibo_i,'weibo_page':weibo_page},
                                             cookies=self.cookie)
            else:
    ...
    def parse_attitude(self, response):
        ...
        html = response.text
        html = json.loads(html)
        next_url = self.start_url.format(weibo_id, self.attitude_page)
        try:
            data = html['data']['data']
        ...
        self.attitude_page += 1
        logging.warning(self.attitude_page)
        if self.attitude_page%20==10:
            self.cookie = json.loads(requests.get(self.cookie_url).text)
        ...
        if int(html['ok']) == 1:
            ...
            yield scrapy.Request(url=next_url, callback=self.parse_attitude, headers=self.headers,cookies=self.cookie,
                                 meta={'weibo_id': weibo_id, 'weibo_page': weibo_page, 'weibo_i': weibo_i})